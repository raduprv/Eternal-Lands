	...
	GLuint *fbo_rbuf)
{
	...
	if (fbo_rbuf != NULL)
	{
		if (*fbo_rbuf != 0)
		{
			ELglDeleteRenderbuffersEXT(1, fbo_rbuf);
		}
		*fbo_rbuf = 0;
	}
	CHECK_GL_ERRORS();
	...
	GLuint * fbo_stencil_buffer, GLuint *fbo_rbuf)
{
	...
	ELglGenRenderbuffersEXT(1, fbo_rbuf);
	CHECK_GL_ERRORS();
	CHECK_FBO_ERRORS();
	ELglBindRenderbufferEXT(GL_RENDERBUFFER_EXT, *fbo_rbuf);
	CHECK_GL_ERRORS();
	CHECK_FBO_ERRORS();
	ELglRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_RGBA8_EXT, width, height);
	CHECK_GL_ERRORS();
	CHECK_FBO_ERRORS();
	ELglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0,
		GL_RENDERBUFFER_EXT, *fbo_rbuf);
	CHECK_GL_ERRORS();
	CHECK_FBO_ERRORS();
