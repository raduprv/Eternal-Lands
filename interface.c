...
void resize_all_root_windows (Uint32 w, Uint32 h)
{
	...
	if (input_widget->window_id != chat_win) {
		widget_resize (input_widget->window_id, input_widget->id, w-hud_x, input_widget->len_y);
		widget_move (input_widget->window_id, input_widget->id, 0, h-input_widget->len_y-hud_y);
	}
}
