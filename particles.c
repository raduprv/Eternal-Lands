			         ...
			         if(dist1*dist1+dist2*dist2<=15*15)
			         	{
							if(particles_list[i]->part_sys_type==TELEPORTER_PARTICLE_SYS)
                     		...
                     		if(particles_list[i]->part_sys_type==TELEPORT_IN_PARTICLE_SYS)
                     		update_teleport_in(particles_list[i]);
                     		else
                     		if(particles_list[i]->part_sys_type==TELEPORT_OUT_PARTICLE_SYS)
                     		update_teleport_out(particles_list[i]);
                     		else
                     		if(particles_list[i]->part_sys_type==BAG_IN_PARTICLE_SYS)
                     		update_bag_in(particles_list[i]);
                     		else
                     		if(particles_list[i]->part_sys_type==BAG_OUT_PARTICLE_SYS)
