...
int display_chat_handler (window_info *win)
{
	...
	draw_string_zoomed (2, 2, &display_text_buffer[line_start], nr_displayed_lines, chat_zoom);
	text_changed = 0;
...
void display_chat ()
{
	if (chat_win <= 0)
	{
		...
		chat_win = create_window ("chat", 0, 0, chat_win_x, chat_win_y, chat_win_len_x, chat_win_len_y, ELW_WIN_DEFAULT & ~ELW_CLOSE_BOX);
		...
		chat_scroll_id = vscrollbar_add_extended (chat_win, chat_scroll_id, NULL, chat_win_len_x-CHAT_WIN_SCROLL_WIDTH, 0, CHAT_WIN_SCROLL_WIDTH, chat_win_len_y, 0, 1.0, 0.77f, 0.57f, 0.39f, 0, 1, scroll_len);
	}
	else
	{
