...
#ifdef FIX_NORMALS
	{
		void* normals_ptr = cur_object->vertex_data + get_normal_offset(cur_object->vertex_options);
		void* vertex_ptr = cur_object->vertex_data + get_vertex_offset(cur_object->vertex_options);
		int k;
		
		// Zero them out.
		printf("%s (%d)\n\n", cur_object->file_name, VERTEX_FLOAT_COUNT); fflush(stdout);
		for(j = 0; j < cur_object->vertex_no; j++)
		{
			printf("%d) %f, %f, %f\n", j, ((float*)(normals_ptr + v_size * j))[0], ((float*)(normals_ptr + v_size * j))[1], ((float*)(normals_ptr + v_size * j))[2]); fflush(stdout);
			((float*)(normals_ptr + v_size * j))[0] = 0.0f;
			((float*)(normals_ptr + v_size * j))[1] = 0.0f;
			((float*)(normals_ptr + v_size * j))[2] = 0.0f;
		}
		printf("\n");

		// Sum up the normals for the faces
		for(j = 0; j < cur_object->index_no / VERTEX_FLOAT_COUNT; j++)
		{
			...
			{
				float v1x = ((float*)(vertex_ptr + v_size * face_list[1]))[0] - ((float*)(vertex_ptr + v_size * face_list[0]))[0];
				float v1y = ((float*)(vertex_ptr + v_size * face_list[1]))[1] - ((float*)(vertex_ptr + v_size * face_list[0]))[1];
				float v1z = ((float*)(vertex_ptr + v_size * face_list[1]))[2] - ((float*)(vertex_ptr + v_size * face_list[0]))[2];
				float v2x = ((float*)(vertex_ptr + v_size * face_list[2]))[0] - ((float*)(vertex_ptr + v_size * face_list[0]))[0];
				float v2y = ((float*)(vertex_ptr + v_size * face_list[2]))[1] - ((float*)(vertex_ptr + v_size * face_list[0]))[1];
				float v2z = ((float*)(vertex_ptr + v_size * face_list[2]))[2] - ((float*)(vertex_ptr + v_size * face_list[0]))[2];
				float nx = v1y * v2z - v1z * v2y;
				float ny = v1z * v2x - v1x * v2z;
				float nz = v1x * v2y - v1y * v2x;
				((float*)(normals_ptr + v_size * face_list[0]))[0] += nx;
				((float*)(normals_ptr + v_size * face_list[0]))[1] += ny;
				((float*)(normals_ptr + v_size * face_list[0]))[2] += nz;
				((float*)(normals_ptr + v_size * face_list[1]))[0] += nx;
				((float*)(normals_ptr + v_size * face_list[1]))[1] += ny;
				((float*)(normals_ptr + v_size * face_list[1]))[2] += nz;
				((float*)(normals_ptr + v_size * face_list[2]))[0] += nx;
				((float*)(normals_ptr + v_size * face_list[2]))[1] += ny;
				((float*)(normals_ptr + v_size * face_list[2]))[2] += nz;
				...
				printf("  %d, %d, %d: %e, %e, %e / %e, %e, %e\n", face_list[0], face_list[1], face_list[2], v1x, v1y, v1z, v2x, v2y, v2z);
				printf("  %e, %e, %e\n\n", nx, ny, nz);
			}
		}
		...
		for(j = 0; j < cur_object->vertex_no; j++)
		{
			float magnitude = sqrt(((float*)(normals_ptr + v_size * j))[0] * ((float*)(normals_ptr + v_size * j))[0] + ((float*)(normals_ptr + v_size * j))[1] * ((float*)(normals_ptr + v_size * j))[1] + ((float*)(normals_ptr + v_size * j))[2] * ((float*)(normals_ptr + v_size * j))[2]) + 0.000001;
			((float*)(normals_ptr + v_size * j))[0] /= magnitude;
			((float*)(normals_ptr + v_size * j))[1] /= magnitude;
			((float*)(normals_ptr + v_size * j))[2] /= magnitude;
			printf("%d) %f, %f, %f (%f)\n", j, ((float*)(normals_ptr + v_size * j))[0], ((float*)(normals_ptr + v_size * j))[1], ((float*)(normals_ptr + v_size * j))[2], magnitude); fflush(stdout);
		}
		printf("\n");
	}
#endif	//FIX_NORMALS
