	...
	bool GlowParticle::idle(const Uint64 delta_t)
	{
		...
		switch (type)
		{
			case GlowEffect::REMOTE_HEAL_GLOW:
			{
				...
				const percent_t scalar = math_cache.powf_05_close(float_time
					* 0.5);
			...
			case GlowEffect::LEVEL_UP_DEFAULT_GLOW:
			{
				...
				const alpha_t scalar =
					math_cache.powf_0_1_rough_close(randfloat(), float_time * 1.0); // smaller numbers -> longer effect
				alpha *= scalar;
				break;
			}
			...
			case GlowEffect::LEVEL_UP_TAI_GLOW_R:
			{
				...
				const alpha_t scalar = 1.0
					- math_cache.powf_0_1_rough_close(randfloat(), float_time * 0.75);
				alpha -= scalar;
				if (alpha < 0.01)
					return false;
				break;
