...
namespace ec
{
	OrbitalMover::OrbitalMover(Effect* _effect, const Vec3 &center): ParticleMover(_effect)
	{
		m_vec3Center = center;
	}
	...
	void OrbitalMover::move(Particle& p, Uint64 usec)
	{
		...
		std::pair<float,float> ps = orbit.getCoordsAtTime( elapsed / 100000.0 + pdata.start_offset,
														  pdata.semimajor_axis ,
														  pdata.eccentricity,
														  pdata.period ) ;
