...
void display_tab_stats ()
{
	if (tab_stats_win <= 0)
	{
		...
		stats_win = tab_add (tab_stats_win, tab_stats_collection_id, "Statistics", 90);
		fill_stats_win ();
		
		knowledge_win = tab_add (tab_stats_win, tab_stats_collection_id, "Knowledge", 80);
		fill_knowledge_win ();

		questlog_win = tab_add (tab_stats_win, tab_stats_collection_id, "Quest log", 80);
		fill_questlog_win ();
...
void display_tab_help ()
{
	if (tab_help_win <= 0)
	{
		...
		help_win = tab_add (tab_help_win, tab_help_collection_id, "Help", 45);
		fill_help_win ();
		
		encyclopedia_win = tab_add (tab_help_win, tab_help_collection_id, "Encyclopedia", 100);
