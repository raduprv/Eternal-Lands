...
void send_new_char(Uint8 * user_str, Uint8 * pass_str, Uint8 * conf_pass_str,
					char skin, char hair, char shirt, char pants, char boots,char head, char type)
{
		int i;
		int j;
		int len;
			...
			switch (in_data[PROTOCOL])
	            {
	              ...
	              case PLAY_SOUND:
	                {
						add_sound_from_server(*((short *)(in_data+3)),*((short *)(in_data+5)),
	              ...
	              case TELEPORT_OUT:
	                {
						...
						add_sound_object("./sound/teleport_out.wav",*((short *)(in_data+3)),*((short *)(in_data+5)),1,0,0);
	                }
	              break;
	              ...
	              case TELEPORT_IN:
	                {
						...
						add_sound_object("./sound/teleport_in.wav",*((short *)(in_data+3)),*((short *)(in_data+5)),1,0,0);
	                }
	              break;
...
void get_message_from_server()
{
	...
	//recived_lenght=SDLNet_TCP_Recv(my_socket, in_data, 4096);
	//recived_lenght=SDLNet_TCP_Recv(my_socket, in_data+put_new_data_offset, 4096);
	...
	//parse the data, and split it in commands
			//data_lenght=*((short *)(in_data+put_new_data_offset+start_my_data_pointer+1));
			data_lenght=*((short *)(in_data+start_my_data_pointer+1));
			data_lenght+=2;//add also the lenght of the lenght field, which is 2
			//process_message_from_server(&in_data[start_my_data_pointer+put_new_data_offset],data_lenght);
			process_message_from_server(&in_data[start_my_data_pointer],data_lenght);
			log_conn(&in_data[start_my_data_pointer],data_lenght);
/*
			if(recived_lenght-start_my_data_pointer<data_lenght)
				{
					//we got incomplete data....
					put_new_data_offset=recived_lenght-start_my_data_pointer;
					//start_my_data_pointer+=data_lenght;
					break;
				}
*/
